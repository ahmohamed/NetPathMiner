	...
	int maxiter) 
{
	...
	// ypre = X %*% B
	F77_NAME(dgemv)(dont_transpose, &nobs, &nx, &double_one, x, &nobs, beta, &int_one, &double_zero, ypre, &int_one);
	...
	while (NotConverged == 1) {
		...
		// btemp = t(x)*(w*(y - ypre))
		F77_NAME(dgemv)(transpose, &nobs, &nx, &double_one, x, &nobs, rss, &int_one, &double_zero, btemp, &int_one);
    ...
    F77_NAME(dgemv)(dont_transpose, &nx, &nx, &double_one, cov, &nx, btemp, &int_one, &double_zero, bret, &int_one);
